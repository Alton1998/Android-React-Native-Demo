...
export default class Input extends Component {
  ...
  handleLogin()
  {
    ...
    axios.post('http://192.168.43.158/api/auth/login/', {
    ...
    .then((response) => {
      ...
      navigate('Fest',{token:this.state.response.data.token})
    })
    .catch((error)=> {
      console.log(error);
      this.setState(
  ...
  render() {
    return (
      <View style={styles.input}>
          ...
          <TouchableOpacity style={[styles.buttonLogin,{top:60}]} onPress={() =>{const {navigate}=this.props.navigation;navigate('SignUp')}}>
              <Text style={styles.buttonText}>Sign Up</Text>
          </TouchableOpacity>
          </View>
          <Text style={{color:'red',position:'absolute',alignSelf:'center',top:300}}>
            {this.state.error}
          </Text>
      </View>
    );
